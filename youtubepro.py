...
with col1:
    ...
    if "Get_state" not in session_state:
        session_state.Get_state = False
    if Get_data or session_state.get("Get_state",False):
        session_state.Get_state = True
...
with col2:
    ...
    # Connect to the new created database
        engine = create_engine('mysql+mysqlconnector://root:sunanda1@localhost/youtube_db', echo=False)
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='sunanda1', db='youtube_db')
cursor = connect_for_question.cursor()

# Q1
if question_tosql == '1. What are the names of all the videos and their corresponding channels?':
    cursor.execute(" SELECT c.Channel_Name, v.Video_Name from video as v join channel as c on v.Playlist_Id=c.Playlist_Id;")
...
# Q3
    ...
    col1,col2 = st.columns(2)
    with col1:
        cursor.execute("select c.Channel_name, v.Video_Name, v.View_count from video as v join channel as c on v.Playlist_Id=c.Playlist_Id order by v.View_count DESC LIMIT 10;")
...
# Q4 
    ...
    cursor.execute(" select Video_Name, Comment_Count from video;")
...
# Q5
    ...
    cursor.execute("SELECT c.Channel_Name, v.Video_Name, v.Like_Count FROM video AS v JOIN channel AS c ON c.Playlist_Id = v.Playlist_Id ORDER BY v.Like_Count DESC LIMIT 5000;")
...
# Q6
    ...
    cursor.execute(" SELECT c.Channel_Name, v.Video_Name, v.Like_Count, v.Dislike_Count FROM video AS v JOIN channel AS c ON c.Playlist_Id = v.Playlist_Id ORDER BY v.Like_Count DESC LIMIT 5000;")
...
# Q8
    ...
    cursor.execute("SELECT c.Channel_Name, v.Video_Name, v.Published_date FROM video AS v JOIN channel AS c ON c.Playlist_Id = v.Playlist_Id WHERE EXTRACT(YEAR FROM Published_date) = 2022;")
...
# Q9
    ...
    cursor.execute("SELECT c.Channel_Name, TIME_FORMAT(SEC_TO_TIME(AVG(TIME_TO_SEC(TIME(v.Duration)))), '%H:%i:%s') AS duration FROM video AS v JOIN channel AS c ON c.Playlist_Id = v.Playlist_Id GROUP by Channel_Name ORDER BY duration DESC;")
...
# Q10
    ...
    cursor.execute("SELECT c.Channel_Name, v.Video_Name, v.Comment_Count FROM video AS v JOIN channel AS c ON c.Playlist_Id = v.Playlist_Id ORDER BY v.Comment_Count DESC;")
